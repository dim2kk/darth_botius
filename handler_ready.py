			...
			if pers in REQS_ALIASES:
				...
				if pers_id in LIST_WITH_LONG_REQS: # сначала пройдем по персонажам со сложными требованиями (легенды, гэс, кам, ват, джлюк)
					...
					if pers_id == "GLREY" or pers_id == "SUPREMELEADERKYLOREN":
						...
						one_char_percentage = 10

					for player in row: # пойдем по очереди по каждому игроку

						if pers_id in player and pers_id != "WATTAMBOR" and pers_id != "KIADIMUNDI": # уже получил персонажа
							already_have.append(player['player_name'])
						else:
							readiness_percent[player['player_name']] = 0

							for pers_name,req in REQS_LONG[pers_id].items():

								if pers_name+"_gear" in player:

									reqq = req.split(" ")

									if reqq[0] == "gear":
										if player[pers_name+"_gear"] >= int(reqq[1]):
											readiness_percent[player['player_name']] += one_char_percentage
									elif reqq[0] == "stars":
										if player[pers_name] >= int(reqq[1]): 
											readiness_percent[player['player_name']] += one_char_percentage
									elif reqq[0] == "power":
										if player[pers_name+"_power"] >= int(reqq[1]): 
											readiness_percent[player['player_name']] += one_char_percentage

					buff = {}
					for p,prc in readiness_percent.items():
						if prc == 0:
							not_ready.append(p)
						elif prc == 99 or prc>100:
							buff[p] = 100
						else:
							buff[p] = prc

					sorted_readiness_percent={}
					sortedList=sorted(buff.values(),reverse=True)

					for sortedKey in sortedList:
					    for key, value in buff.items():
					        if value==sortedKey:
					            sorted_readiness_percent[key]=value

					msg = f'(обновлено с сайта swgoh.gg *{passed_time_text} назад*)\n\n'
					msg += f'У меня есть информация о `{count_row}` игроках гильдии\n\n'

					if len(already_have):
						msg += f'Из них *{len(already_have)}* уже имеют данного персонажа на `7` звезд:\n_{", ".join(already_have)}_\n\n'

					msg += f'Готовность к персонажу *{LEGENDARIES[pers_id]}*:\n\n'

					for p,prc in sorted_readiness_percent.items():
						
						rounded_percent = round(prc,-1)

						dec = rounded_percent/10
						full_sectors = int(dec)
						empty_sectors = int(10-dec)

						for i in range(full_sectors):
							msg += f'▰'
						for j in range(empty_sectors):
							msg += f'▱'
